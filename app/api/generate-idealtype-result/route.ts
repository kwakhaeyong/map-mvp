...
export async function POST(request: NextRequest) {
  ...
  if (!acquiredMarker) {
    ...
    const cachedAfterWait = await waitForCachedGeneration<IdealTypeResult>(cacheKey);
    const waitedMs = Date.now() - waitStartedAt;
    if (cachedAfterWait) {
      logGenerationMarker({ topic: "idealType", outcome: "wait_hit", waitedMs });
      cacheStatus = "hit";
    ...
    logGenerationMarker({ topic: "idealType", outcome: "wait_timeout", waitedMs });
  } else {
    logGenerationMarker({ topic: "idealType", outcome: "acquired", waitedMs: 0 });
  }
