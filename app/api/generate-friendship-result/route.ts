...
import { generateFriendshipResult } from "../../../src/map-decision-v1/engine/friendship-generator";
import {
...
import { FriendshipResult, MapSession } from "../../../src/map-decision-v1/types";
...
const SIGNATURE_SCOPE = "friendship";

type RequestBody = { session: MapSession };
type SuccessResponse = { result: FriendshipResult; signature: string | null };
...
// 정리한다는 점)는 maxIdealTypeMessages() 주석 참고.
function maxFriendshipMessages(): number {
  const axisCount = (resolveTopic("friendship").axes ?? []).length;
  return axisCount * 2 * 2 + 10;
}

function isOversized(session: MapSession): boolean {
  if (!Array.isArray(session.messages)) return true;
  if (session.messages.length > maxFriendshipMessages()) return true;
...
function requiredAnswerCount(): number {
  const axes = resolveTopic("friendship").axes ?? [];
...
export async function POST(request: NextRequest) {
  ...
  const logTiming = (outcome: string): void =>
    logGenerationRequest({ topic: "friendship", requestStartedAt, cache: cacheStatus, outcome });
  ...
  const cacheKey = computeGenerationCacheKey("friendship", session);
  const cachedBeforeReservation = await getCachedGeneration<FriendshipResult>(cacheKey);
  ...
  if (!acquiredMarker) {
    ...
    const cachedAfterWait = await waitForCachedGeneration<FriendshipResult>(cacheKey);
    const waitedMs = Date.now() - waitStartedAt;
    if (cachedAfterWait) {
      logGenerationMarker({ topic: "friendship", outcome: "wait_hit", waitedMs });
      cacheStatus = "hit";
    ...
    logGenerationMarker({ topic: "friendship", outcome: "wait_timeout", waitedMs });
  } else {
    logGenerationMarker({ topic: "friendship", outcome: "acquired", waitedMs: 0 });
  }
  ...
  try {
    ...
    // 이상형·나 소개·진로와 같은 레이트리밋 예산을 공유한다(엔드포인트만
    ...
    const { result, countsAsFailure } = await generateFriendshipResult(session);
    if (!result) {
