...
import { generateWorkResult } from "../../../src/map-decision-v1/engine/work-generator";
import {
...
import { MapSession, WorkResult } from "../../../src/map-decision-v1/types";
...
const SIGNATURE_SCOPE = "work";

type RequestBody = { session: MapSession };
type SuccessResponse = { result: WorkResult; signature: string | null };
...
// app/api/generate-friendship-result/route.ts의 maxFriendshipMessages()와
...
// 재발하지 않는다.
function maxWorkMessages(): number {
  const axisCount = (resolveTopic("work").axes ?? []).length;
  return axisCount * 2 * 2 + 10;
}

function isOversized(session: MapSession): boolean {
  if (!Array.isArray(session.messages)) return true;
  if (session.messages.length > maxWorkMessages()) return true;
...
export async function POST(request: NextRequest) {
  ...
  const logTiming = (outcome: string): void =>
    logGenerationRequest({ topic: "work", requestStartedAt, cache: cacheStatus, outcome });
  ...
  const cacheKey = computeGenerationCacheKey("work", session);
  const cachedBeforeReservation = await getCachedGeneration<WorkResult>(cacheKey);
  ...
  if (!acquiredMarker) {
    ...
    const cachedAfterWait = await waitForCachedGeneration<WorkResult>(cacheKey);
    const waitedMs = Date.now() - waitStartedAt;
    if (cachedAfterWait) {
      logGenerationMarker({ topic: "work", outcome: "wait_hit", waitedMs });
      cacheStatus = "hit";
    ...
    logGenerationMarker({ topic: "work", outcome: "wait_timeout", waitedMs });
  } else {
    logGenerationMarker({ topic: "work", outcome: "acquired", waitedMs: 0 });
  }
  ...
  try {
    ...
    const { result, countsAsFailure } = await generateWorkResult(session);
    if (!result) {
