...
import { generateTravelResult } from "../../../src/map-decision-v1/engine/travel-generator";
import {
...
import { MapSession, TravelResult } from "../../../src/map-decision-v1/types";
...
const SIGNATURE_SCOPE = "travelStyle";

type RequestBody = { session: MapSession };
type SuccessResponse = { result: TravelResult; signature: string | null };
...
// app/api/generate-taste-result/route.ts의 maxTasteMessages()와 완전히
...
function maxTravelMessages(): number {
  const axisCount = (resolveTopic("travelStyle").axes ?? []).length;
  return axisCount * 2 * 2 + 10;
}

function isOversized(session: MapSession): boolean {
  if (!Array.isArray(session.messages)) return true;
  if (session.messages.length > maxTravelMessages()) return true;
...
function requiredAnswerCount(): number {
  const axes = resolveTopic("travelStyle").axes ?? [];
...
export async function POST(request: NextRequest) {
  ...
  const logTiming = (outcome: string): void =>
    logGenerationRequest({ topic: "travelStyle", requestStartedAt, cache: cacheStatus, outcome });
  ...
  const cacheKey = computeGenerationCacheKey("travelStyle", session);
  const cachedBeforeReservation = await getCachedGeneration<TravelResult>(cacheKey);
  ...
  if (!acquiredMarker) {
    ...
    const cachedAfterWait = await waitForCachedGeneration<TravelResult>(cacheKey);
    const waitedMs = Date.now() - waitStartedAt;
    if (cachedAfterWait) {
      logGenerationMarker({ topic: "travelStyle", outcome: "wait_hit", waitedMs });
      cacheStatus = "hit";
    ...
    logGenerationMarker({ topic: "travelStyle", outcome: "wait_timeout", waitedMs });
  } else {
    logGenerationMarker({ topic: "travelStyle", outcome: "acquired", waitedMs: 0 });
  }
  ...
  try {
    ...
    // 이상형·나 소개·친구·일할 때의 나·취향·진로와 같은 레이트리밋 예산을
    ...
    const { result, countsAsFailure } = await generateTravelResult(session);
    if (!result) {
