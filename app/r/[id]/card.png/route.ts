...
import { FriendshipResult, IdealTypeResult, SelfIntroResult, TasteResult, TravelResult, WorkResult } from "../../../../src/map-decision-v1/types";
import {
...
import { buildTravelCardElement } from "../../../../src/map-decision-v1/engine/travel-card-image";
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // 나 소개·성격, 친구·인간관계, 일할 때의 나, 취향, 여행 스타일은
  // 초대장 컨셉 하나만 지원한다(테마 파라미터 없음) — 이상형처럼 여러
  // 테마를 지원할 필요가 없었다.
  ...
  if (share.record.resultLayoutId === "travelStyle") {
    if (!isTravelResult(share.record.result)) return new Response("Not found", { status: 404 });
    const response = new ImageResponse(buildTravelCardElement(share.record.result), {
      width: INVITATION_CARD_WIDTH,
      height: INVITATION_CARD_HEIGHT,
      fonts: loadInvitationFonts(),
    });
