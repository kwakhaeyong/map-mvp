...
- selfReflection(strengths/blindSpots): ★가장 중요한 항목★. strengths는 사용자가 일할 때 실제로 발휘하고 있는 것을 답변에서 근거를 찾아 짚어준다. blindSpots는 여러 답변을 교차했을 때 드러나는 모순이나 반복되는 약점을 짚는다 — 마지막 문항의 자기평가와 앞선 행동 답변 사이의 간극(또는 일치)을 최소 1개는 반드시 포함한다(위 ★가장 중요한 재해석★ 참고). 짧게 요약하면 성립하지 않는 통찰이니 필요한 만큼 충분히 써라. 각 2~4개.
