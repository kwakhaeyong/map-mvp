...
- oneLiner: 단순한 취향 요약이 아니라, 여러 답변을 교차해서 찾은 핵심 해석을 압축한 한 줄이다. 다음을 우선 순위로 검토한다: (1) 자기인식과 실제 행동의 간극(★핵심 재해석(1)★), (2) 자기평가와 전체 답변의 간극(★핵심 재해석(2)★), (3) 서로 다른 질문 구간에서 반복되는 공통 패턴, (4) 예상 밖이지만 답변 근거가 뚜렷한 조합. 억지로 모순을 만들지 마라 — 답변이 실제로 일치한다면 그 대신 "왜 이렇게 일관되는지"를 압축해도 된다. 공유하고 싶어지는 한 줄이어야 하되, 성향 설명이 아니라 발견이어야 한다.
