...
★핵심 재해석 (1)★: 1번 문항("혼자 있는 시간에 나는 주로 뭘 해?")은 딱 하나만 고르는 자기인식 답변이고, 2번 문항("최근 일주일, 실제로 시간을 가장 많이 쓴 건?")은 최대 3개까지 고를 수 있는 실제 사용 기록이다(2번에서 여러 개를 골랐다면 그중 1순위, 즉 가장 먼저 고른 것과 비교하라). 이 둘 사이의 간극이 이 결과의 핵심 재료 중 하나다 — 예를 들어 1번에서 "읽는 편"이라고 답했는데 2번의 1순위가 "영상 보기"처럼 실제로는 다른 방식으로 시간을 썼다면, 그 간극을 반드시 짚어라(자기 이미지와 실제 습관이 다르다는 뜻이다). 두 답이 실제로 같은 방향이라면(예: 1번 "읽는 편" + 2번 1순위 "책·글 읽기") 억지로 간극을 만들지 말고, 대신 왜 그 두 답이 서로 맞아떨어지는지를 근거로 짚어라.

★핵심 재해석 (2)★: 마지막 문항("나는 취향이 뚜렷한 사람인 것 같아?")은 사용자가 스스로를 평가한 유일한 문항이다 — 나머지 19개는 전부 구체적인 선호·행동을 물었을 뿐, "내 취향은 이렇다"라고 직접 말한 적이 없다. 이 자기평가와 앞선 19개 답변 사이의 간극(또는 일치)을 짚는 것이 이 결과의 또 다른 핵심 재해석 축이다. 예를 들어 "잘 모르겠다"고 답했는데 실제로는 분위기·이야기·스타일 답변이 뚜렷하게 한 방향으로 모여 있었다면 그 간극을, 반대로 "그런 편인 것 같다"고 답했는데 정작 답변들이 서로 다른 방향으로 흩어져 있었다면 그 간극을 selfReflection에서 반드시 짚어라. 자기평가와 답변들이 실제로 서로 맞아떨어지는 경우라면, 그 대신 왜 그 자기평가가 답변들과 맞아떨어지는지를 근거로 짚어라.
...
- patterns: ★핵심★. 사용자가 고른 선택지들을 가로질러 반복되는 취향의 결을 짚어준다. 단순 요약이 아니라 "왜 그런 결이 반복되는지"에 대한 해석을 담는다. 서로 다른 구간(시간 사용·끌림·온도·자기인식 등)의 답변을 최소 2개 이상 교차해서 나온 패턴을 최소 1개는 반드시 반영한다. 항목마다 문장을 시작하는 골격이 겹치지 않게 접근 방식을 섞어라 — 전부 "~라고 답했지만 정작"류의 간극형 문장으로만 채우지 말고, 반복되는 태도 자체를 짚는 문장, 상황에 따라 달라지는 모습을 짚는 문장, 서로 다른 답변을 연결한 문장처럼 관점을 다양하게 써라. 2~4개.
...
- selfReflection(awareness/blindSpots): ★가장 중요한 항목★. awareness는 사용자가 스스로도 어느 정도 알고 있을 법한, 답변에서 근거를 찾아 짚어주는 특징이다. blindSpots는 여러 답변을 교차했을 때 드러나는, 본인은 잘 모를 수 있는 결이다 — 위 ★핵심 재해석 (1)★과 ★핵심 재해석 (2)★ 중 최소 하나는 반드시 포함한다. 짧게 요약하면 성립하지 않는 통찰이니 필요한 만큼 충분히 써라. 각 2~4개.
...
async function attemptGeneration(client: Anthropic, session: MapSession, maxTokens: number): Promise<{ result: TasteResult | null; truncated: boolean; countsAsFailure: boolean }> {
