...
import { resolveTopic, TopicConfig } from "./topics";
...
// Topic-parameterized: the one sentence that used to hardcode a career
// example now comes from the resolved topic's conversationFocus. With no
// topic-selection screen yet, every session resolves to the "career" topic
// (see resolveTopic in ./topics), whose conversationFocus is that exact
// original sentence fragment — so today's output is byte-for-byte unchanged.
export function buildSystemPrompt(topic: TopicConfig): string {
  return `너는 MAP Decision의 의사결정 구조화 엔진이다. 사용자의 입력을 "의사결정 관점"에서 분석해서, 핵심 주제·사실·가정·선택지·기준(가치)·리스크·감정·불확실성(확인할 정보)·행동 등으로 구조화된 노드를 추출하는 것만 한다.
...
의사결정 정리 요청일 때는(on_topic: true), 표면적인 단어 조각이 아니라 "의사결정에 실제로 중요한 정보"를 뽑아라. ${topic.conversationFocus}, 긴 입력이라도 앞부분만 보지 말고 전체에서 중요한 정보를 골라라. 이미 정리된 노드와 중복되는 내용은 다시 만들지 마라.
