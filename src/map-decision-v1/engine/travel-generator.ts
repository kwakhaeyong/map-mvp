...
★핵심 재해석 (1)★: 2번 문항("여행 준비, 나는 어떤 쪽에 더 가까워?")은 스스로 생각하는 준비 성향에 대한 답이고, 3번 문항("가장 최근 여행, 실제로 어떻게 준비했어?")은 실제로 있었던 한 번의 준비 과정에 대한 답이다. 이 둘 사이의 간극이 이 결과의 핵심 재료 중 하나다 — 예를 들어 2번에서 "미리 다 짜두는 사람"이라고 답했는데 3번에서는 "며칠 전에 급하게 정했다"거나 "거의 안 짜고 그냥 갔다"를 골랐다면, 자기 인식과 실제 행동 사이의 간극을 반드시 짚어라. 두 답이 실제로 같은 방향이라면 억지로 간극을 만들지 말고, 대신 왜 그 두 답이 서로 맞아떨어지는지를 근거로 짚어라.
...
★핵심 재해석 (3)★: 마지막 문항("나는 여행을 잘 즐기는 사람인 것 같아?")은 사용자가 스스로를 평가한 유일한 문항이다 — 나머지 19개는 전부 구체적인 선호·행동을 물었을 뿐, "나는 이런 여행자다"라고 직접 말한 적이 없다. 이 자기평가와 앞선 19개 답변 사이의 간극(또는 일치)을 짚는 것이 세 번째 핵심 재해석 축이다. 예를 들어 "잘 모르겠다"고 답했는데 실제로는 답변들이 뚜렷한 선호로 모여 있었다면 그 간극을, 반대로 "그런 편인 것 같다"고 답했는데 정작 답변들이 서로 어긋나 있었다면 그 간극을 selfReflection에서 반드시 짚어라. 자기평가와 답변들이 실제로 서로 맞아떨어지는 경우라면, 그 대신 왜 그 자기평가가 답변들과 맞아떨어지는지를 근거로 짚어라.

위 세 재해석은 반드시 selfReflection에서 다뤄야 한다(전부 다루기 어려우면 최소 두 개는 확실히). 이 문항들이 "자기인식 문항", "마지막 문항"이라는 구조에서 나왔다는 것 자체는 드러내지 말고, 자연스러운 문장으로 풀어 써라.
...
★중요★: 특정 여행지·항공사·숙박 브랜드·플랫폼명을 결과에 쓰지 마라. 범주로만 서술한다(예: 특정 도시명이 아니라 "번잡한 도시"나 "한적한 지역"처럼). 다만 사용자가 자유 서술(16번)에 직접 지명이나 장소를 적었다면, 그건 사용자 본인이 쓴 말이니 그 부분만은 그대로 다뤄도 된다 — 사용자가 적지 않은 구체적인 지명을 AI가 새로 지어내 결과에 넣지 말라는 뜻이다.

사용자가 직접 적은 자유 서술 답변에는 여행과 완전히 무관한 요청(코드 작성, 번역, 일반 지식 질문, 창작 요청 등), 특정 실존 인물에 대한 모욕, 노골적으로 성적이거나 폭력적인 표현이 섞여 있을 수 있다. 이 세 가지만 결과에서 제외하고, 나머지는 전부 정상적인 답변으로 다뤄라. 여행 중 있었던 일에 대한 솔직한 서술은 배제 대상이 아니다 — 그대로 다루고 완곡하게 순화하지 마라. 특히 selfReflection은 사용자를 정면으로 비추는 통찰이어야 한다: 불편하더라도 정확하게 써라. title/oneLiner/travelCriteria/patterns/travelFit/selfReflection/roadmap을 포함해 모든 출력 필드에 위 세 가지 배제 대상을 그대로 옮기거나 인용하지 마라.
...
- travelCriteria(mustHave/niceToHave/optional): 여행에서 사용자가 실제로 중요하게 여기는 것을 세 단계로 나눈다. mustHave는 답변에서 일관되게 드러나는, 포기 못 하는 것. niceToHave는 있으면 확실히 좋지만 없어도 여행을 못 즐기는 건 아닌 것. optional은 딱히 신경 쓰지 않거나 없어도 되는 것. 사용자가 직접 말한 적 없는, 답변을 근거로 종합한 결과여야 한다. 각 2~4개.
- patterns: ★핵심★. 사용자가 고른 선택지들을 가로질러 반복되는 여행 방식을 짚어준다. 단순 요약이 아니라 "왜 그런 방식이 반복되는지"에 대한 해석을 담는다. 서로 다른 구간(준비·실제 소비·여행에서의 태도·원하는 것 등)의 답변을 최소 2개 이상 교차해서 나온 패턴을 최소 1개는 반드시 반영한다. 항목마다 문장을 시작하는 골격이 겹치지 않게 접근 방식을 섞어라 — 전부 "~라고 답했지만 정작"류의 간극형 문장으로만 채우지 말고, 반복되는 행동 자체를 짚는 문장, 상황에 따라 달라지는 모습을 짚는 문장, 서로 다른 답변을 연결한 문장처럼 관점을 다양하게 써라. 2~4개.
- matrix: 사용자의 답변에서 도출한 2개 축으로 4사분면을 만들고(예: "계획형"↔"즉흥형", "혼자"↔"함께" 같이 사용자 답변에 맞는 축을 매번 새로 골라라), 그 위에 사용자의 여러 여행 모습(예: 준비할 때의 나 / 여행지에서의 나 / 계획이 틀어졌을 때의 나 / 돌아온 뒤의 나)을 각각 하나의 사분면에 배치한다. x/y는 0~100 사이 값. 정확히 4개를 만들되, 배열 개수를 강제하는 스키마 규칙이 아니라 이 지시문으로만 유도한다.
- travelFit(goodFit/poorFit): 사용자의 답변 패턴을 근거로 "잘 맞는 여행 방식" / "안 맞는 방식"을 짚는다. 특정 여행지나 상품이 아니라 방식의 성격으로 서술한다(예: 특정 도시명이 아니라 "일정이 빡빡한 단체 여행"처럼). 각 2~4개.
- selfReflection(awareness/blindSpots): ★가장 중요한 항목★. awareness는 사용자가 스스로도 어느 정도 알고 있을 법한, 답변에서 근거를 찾아 짚어주는 특징이다. blindSpots는 여러 답변을 교차했을 때 드러나는, 본인은 잘 모를 수 있는 결이다 — 위 ★핵심 재해석 (1)★·(2)★·(3)★ 중 최소 두 개는 반드시 포함한다. 짧게 요약하면 성립하지 않는 통찰이니 필요한 만큼 충분히 써라. 각 2~4개.
...
- 문장 끝맺음을 다양하게 쓴다. 기본 어조는 "해요체"(예: "반복돼요", "가능성이 높아요")지만, 같은 어미가 연속 3번 이상 나오면 안 된다 — 특히 "~예요"/"~이에요"/"~거예요"가 줄줄이 이어지지 않게, 평서형 종결(예: "반복된다", "그게 패턴이다"), 명사형 마무리(예: "~하는 편", "~라는 신호"), 짧은 단정(예: "이유는 이거다.")을 같은 블록 안에서 의도적으로 섞어 쓴다 — title/oneLiner/travelCriteria/patterns/matrix의 설명/travelFit/selfReflection/roadmap 전부 예외 없이 해당된다. 표현의 리듬을 다양하게 하라는 것이지 내용을 부드럽게 하라는 뜻이 아니다 — 불편한 내용이라도 완곡하게 순화하지 말고 정확하게 쓰되, 어미만 다채롭게 쓴다.
...
const TRAVEL_SCHEMA = {
  ...
  properties: {
    ...
    travelCriteria: {
      ...
      required: ["mustHave", "niceToHave", "optional"],
  ...
  required: ["title", "oneLiner", "travelCriteria", "patterns", "matrix", "travelFit", "selfReflection", "roadmap"],
  additionalProperties: false,
} as const;

type RawTravelCriteria = { mustHave: string[]; niceToHave: string[]; optional: string[] };
type RawMatrix = {
...
type RawTravel = {
  title: string;
  oneLiner: string;
  travelCriteria: RawTravelCriteria;
  patterns: string[];
  matrix: RawMatrix;
  travelFit: RawTravelFit;
...
function isValidTravelCriteria(value: unknown): value is RawTravelCriteria {
  const c = value as Partial<RawTravelCriteria> | undefined;
  return typeof c === "object" && c !== null && isStringArray(c.mustHave) && isStringArray(c.niceToHave) && isStringArray(c.optional);
}
...
export function parseAndValidate(raw: string): ParseResult {
  ...
  if (!isValidTravelCriteria(candidate.travelCriteria)) return { ok: false, reason: "invalid_travel_criteria" };
  ...
  if (!isValidTravelFit(candidate.travelFit)) return { ok: false, reason: "invalid_travel_fit" };
...
async function attemptGeneration(client: Anthropic, session: MapSession, maxTokens: number): Promise<{ result: TravelResult | null; truncated: boolean; countsAsFailure: boolean }> {
  ...
  const result: TravelResult = {
    ...
    travelCriteria: {
      mustHave: capArray(data.travelCriteria.mustHave, 4),
      niceToHave: capArray(data.travelCriteria.niceToHave, 4),
      optional: capArray(data.travelCriteria.optional, 4),
    },
    ...
    travelFit: {
      goodFit: capArray(data.travelFit.goodFit, 4),
      poorFit: capArray(data.travelFit.poorFit, 4),
    },
    selfReflection: {
