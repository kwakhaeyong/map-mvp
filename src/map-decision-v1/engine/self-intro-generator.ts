...
- patterns: ★핵심★. 사용자가 고른 선택지들을 가로질러 반복되는 행동 패턴을 짚어준다. 단순 요약이 아니라 "왜 그런 패턴이 반복되는지"에 대한 해석을 담는다. 서로 다른 영역의 답변을 최소 2개 이상 교차해서 나온 패턴을 최소 1개는 반드시 반영한다. 항목마다 문장을 시작하는 골격이 겹치지 않게 접근 방식을 섞어라 — 두 답을 대조하듯 가르는 간극형 문장으로만 채우지 말고, 반복되는 행동 자체를 짚는 문장, 상황에 따라 달라지는 모습을 짚는 문장, 서로 다른 답변을 연결한 문장처럼 관점을 다양하게 써라. 간극을 짚는 문장 자체는 계속 써도 된다 — 골격이 반복되는 것만 피하면 되고, 짚을 때도 두 모습이 공존하는 것으로 자연스럽게 풀어 써라. 2~4개. 위 ★반복 금지★ 원칙이 이 골격 다양화 지시보다 우선한다 — 먼저 반복 금지 원칙에 따라 이 블록에 남길 결을 추린 뒤, 그렇게 남은 항목들 사이에서만 문장 골격을 다양화하라. 골격을 다양하게 채우겠다고 selfReflection에 넘겨야 할 결(답변 사이의 모순·긴장)을 이 블록에 욱여넣지 말고, 반대로 이 블록에서 이미 다룬 결을 골격만 바꿔 selfReflection에 다시 밀어넣지도 마라.
- matrix: 사용자의 답변에서 도출한 2개 축으로 4사분면을 만들고(예: "신중함"↔"즉흥성", "표현함"↔"절제함" 같이 사용자 답변에 맞는 축을 매번 새로 골라라), 그 위에 사용자 자신의 여러 면모(예: 쉴 때의 나 / 갈등할 때의 나 / 낯선 사람 앞에서의 나 / 가까운 사람 앞에서의 나)를 각각 하나의 사분면에 배치한다. x/y는 0~100 사이 값. 정확히 4개를 만들되, 배열 개수를 강제하는 스키마 규칙이 아니라 이 지시문으로만 유도한다.
