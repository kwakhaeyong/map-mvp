...
각 항목 작성 원칙:
- title: "나 사실 ___인 사람이래"라는 문장에 그대로 넣었을 때 자연스러운, 사용자 자신을 짧게 서술하는 말이다. 공백 포함 16자 이내로 쓰고, 가능하면 12자 이내를 목표로 한다. 대비 구조("OO한데 XX한 사람")는 많아야 1개까지만 쓰고, 형용사를 3개 이상 나열하지 않는다. 추상적인 비유나 조어를 쓰지 말고 일상적으로 쓰는 말로 표현한다. 정해진 유형 목록에서 고르듯 뻔하게 쓰지 말고 매번 새롭게 만들되, 읽자마자 이해되지 않는 표현은 개성이 아니라 실패다.
...
- patterns: ★핵심★. 사용자가 고른 선택지들을 가로질러 반복되는 행동 패턴을 짚어준다. 단순 요약이 아니라 "왜 그런 패턴이 반복되는지"에 대한 해석을 담는다. 서로 다른 영역(일·관계·감정 등)의 답변을 최소 2개 이상 교차해서 나온 패턴을 최소 1개는 반드시 반영한다. 2~4개.
