...
퀴즈는 총 34문항이고 전부 필수다. 전부 "실제로 있었던 일에서 실제로 어떻게 행동했는지"만 묻는다 — "당신은 계획적인 사람인가요?" 같은 추상적인 자기평가 문항은 하나도 없다. 문항은 관계에서 원하는 것·생활 방식·갈등 대처(가까운 사람과 갈등이 있을 때 스트레스를 어떻게 푸는지)·연애/가까운 관계가 오래갈 때의 패턴 등 4개 축(이 4개는 태그를 정하는 축이다 — 아래에서 별도로 설명한다)과, 관계가 깊어지는 순간의 행동·실수 대응·친구·동료와의 의견 차이·결정 방식·감정 표현·모임에서 맡는 역할·후회했던 선택·쉴 때의 모습·소통 방식·달라진 계기·낯선 사람을 대하는 태도 등 훨씬 다양한 실생활 영역을 다룬다. 경험형 문항 3개(실수했던 순간·돌아보면 아쉬웠던 선택·예전과 달라진 점) 바로 뒤에는 "그게 언제였고 그때 실제로 어떻게 했는지"를 한 줄로 적는 자유 서술이 붙어있다(건너뛸 수 있지만, 답했다면 그 경험 답변의 실제 배경을 사용자가 직접 설명한 것이니 가장 근거가 확실한 재료다) — 있다면 patterns와 특히 selfReflection에서 반드시 근거로 활용하라.
