...
★새 원칙 — 반복 금지★: 같은 발견을 여러 블록에서 반복해서 쓰지 마라. 여섯 블록은 서로 다른 역할을 맡는다 — coreValues는 답변 전체를 근거로 꼭 지키는 것/중요하게 여기는 것/유연하게 넘어가는 것을 분류하고, patterns는 서로 다른 영역(일·관계·감정 등)의 답변을 교차했을 때 왜 그런 행동 패턴이 반복되는지를 해석하고, matrix는 그 패턴이 여러 상황(일할 때·쉴 때·갈등할 때·낯선 사람 앞에서 등)에서 어떻게 다르게 나타나는지를 위치로 보여주고, traits는 그 패턴을 바탕으로 타인이 참고하면 좋은 강점과 부딪힐 수 있는 주의점을 짚고, selfReflection은 그 패턴이 채워주는 필요와, 서로 다른 답변 사이에서 드러나는 모순이나 긴장을 다루고, roadmap은 그래서 무엇을 해볼지 구체적인 행동을 담는다. 특히 patterns와 selfReflection이 겹치기 가장 쉽다 — patterns에서 이미 짚은 결을 selfReflection에서 같은 근거·같은 문장으로 다시 쓰지 마라. 최종 결론이 두 블록에서 겹치더라도 그 결론에 도달하는 근거나 적용 범위는 블록마다 달라야 한다 — patterns에 이미 쓴 결이라면 selfReflection에서는 아예 다른 답변 조합에서 나온 다른 결을 찾아 써라.

각 항목 작성 원칙:
- title: "나 사실 ___인 사람이래"라는 문장에 그대로 넣었을 때 자연스러운, 사용자 자신을 짧게 서술하는 말이다. 공백 포함 16자 이내로 쓰고, 가능하면 12자 이내를 목표로 한다. "다정한", "따뜻한", "든든한" 같은 일반적인 성격 형용사만으로 title을 채우지 마라 — 누구에게나 붙일 수 있는 말은 그 사람을 가리키지 못한다. 답변에 나온 구체적인 행동·상황·선택을 반영해서, 다른 사람의 결과에는 그대로 붙지 않을 표현을 만들어라. "X한데 속은 Y한", "X하지만 Y한" 같은 대비 구조를 기본값으로 쓰지 마라 — 답변에서 대비가 실제로 두드러질 때만 쓰고, 쓰더라도 많아야 1개까지다. 형용사를 3개 이상 나열하지 않는다. 추상적인 비유나 조어를 쓰지 말고 일상적으로 쓰는 말로 표현한다. 정해진 유형 목록에서 고르듯 뻔하게 쓰지 말고 매번 새롭게 만들되, 읽자마자 이해되지 않는 표현은 개성이 아니라 실패다. title은 반드시 명사 또는 명사구로 끝나야 한다 — "나 사실 ___인 사람이래"에 그대로 넣었을 때 문장이 완결돼야 하기 때문이다. "~는", "~한", "~던"처럼 관형절로 끝나서 뒤에 명사가 빠진 채 마무리되면 안 된다. 좋은 예: "돌다리도 두들기고 건너는 신중파", "먼저 손 내미는 다정러". 나쁜 예: "돌다리도 두들기고 건너는"(뒤에 명사가 없어 문장이 안 끝난다), "먼저 손 내미는"(마찬가지).
...
- patterns: ★핵심★. 사용자가 고른 선택지들을 가로질러 반복되는 행동 패턴을 짚어준다. 단순 요약이 아니라 "왜 그런 패턴이 반복되는지"에 대한 해석을 담는다. 서로 다른 영역(일·관계·감정 등)의 답변을 최소 2개 이상 교차해서 나온 패턴을 최소 1개는 반드시 반영한다. 항목마다 문장을 시작하는 골격이 겹치지 않게 접근 방식을 섞어라 — 두 답을 대조하듯 가르는 간극형 문장으로만 채우지 말고, 반복되는 행동 자체를 짚는 문장, 상황에 따라 달라지는 모습을 짚는 문장, 서로 다른 답변을 연결한 문장처럼 관점을 다양하게 써라. 간극을 짚는 문장 자체는 계속 써도 된다 — 골격이 반복되는 것만 피하면 되고, 짚을 때도 두 모습이 공존하는 것으로 자연스럽게 풀어 써라. 2~4개. 위 ★반복 금지★ 원칙이 이 골격 다양화 지시보다 우선한다 — 먼저 반복 금지 원칙에 따라 이 블록에 남길 결을 추린 뒤, 그렇게 남은 항목들 사이에서만 문장 골격을 다양화하라. 골격을 다양하게 채우겠다고 selfReflection에 넘겨야 할 결(답변 사이의 모순·긴장)을 이 블록에 욱여넣지 말고, 반대로 이 블록에서 이미 다룬 결을 골격만 바꿔 selfReflection에 다시 밀어넣지도 마라.
...
- selfReflection(whatYouOffer/whatToImprove): ★가장 중요한 항목★. whatYouOffer는 patterns에서 이미 다룬 패턴 자체나 그 이유를 다시 확인해주는 자리가 아니다 — patterns가 "무엇이 반복되는지"를 다뤘다면, whatYouOffer는 그 패턴이 관계·일에서 실제로 어떤 역할을 하는지, 즉 채워주는 심리적·실용적 필요(예: 안정감, 통제감, 신뢰, 효능감, 정체성 표현 등)를 답변을 근거로 짚는다. "그런 편이죠"처럼 이미 아는 내용을 확인해주는 문장으로 끝내지 마라 — patterns와 다른 질문("그게 왜 필요한지")에 답해야 한다. whatToImprove는 여러 답변을 교차했을 때 드러나는 모순이나 반복되는 약점을 짚는다 — 예: "갈등이 생기면 거리를 둔다"고 답했으면서 "가까운 사람이 힘들 때는 옆에서 챙긴다"고도 답했다면, 이 둘 사이의 긴장을 짚는 식이다. 짧게 요약하면 성립하지 않는 통찰이니 필요한 만큼 충분히 써라. 각 2~4개.
...
export type SelfIntroGenerationOutcome = { result: SelfIntroResult | null; countsAsFailure: boolean };
...
// 최종적으로도 카운트한다.
export async function generateSelfIntroResult(session: MapSession): Promise<SelfIntroGenerationOutcome> {
