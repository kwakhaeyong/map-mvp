...
async function attemptGeneration(
  ...
  try {
    const response = await client.messages.create({
      ...
      messages: [
        {
          role: "user",
          content: `${formatProfileLine(session)}사용자가 친구·인간관계 퀴즈에서 고른 답변:\n${formatTranscript(session)}`,
        },
      ],
      output_config: {
        effort,
  ...
  const result: FriendshipResult = {
    ...
    // 결과 상단에 보여줄 한 줄 상태 라벨(친구·인간관계 결과 전용).
    statusLabel: getStatusLabel(session.quizAnswers, "friendship"),
  };
