...
function isValidIdealTypeResultShape(value: unknown): boolean {
  ...
  // 없어도 통과시키고, 있으면 5개 부품 필드가 전부 짧은 문자열인지만
  // 확인한다(태그와 같은 이유로 고정 사전 값인지까지는 재검증하지 않는다).
  if (r.silhouette !== undefined) {
    ...
    if (
      !isShortString(silhouette.hair) ||
      !isShortString(silhouette.top) ||
      !isShortString(silhouette.pose) ||
      !isShortString(silhouette.accessory) ||
      !isShortString(silhouette.color)
    )
      return false;
  }
