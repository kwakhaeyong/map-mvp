...
퀴즈는 두 구간으로 나뉜다: 반드시 답하는 필수 37문항(빠른 탭형 8개 — 대부분 외모·스타일·말투 등 이상형에 대한 단순한 취향을 한눈에 골라 답하는 질문이지만, 그중 2개(요즘 마음 상태, 관계에서 주로 맡는 역할)는 이상형이 아니라 지금 사용자 자신의 상태·역할을 묻는 질문이다 / 순위 매기기형 2개 — 하나는 마음을 표현받는 방식 여러 개를 좋아하는 순서대로 매긴 질문(이상형에게 바라는 것), 다른 하나는 반대로 사용자 자신이 마음을 표현할 때 실제로 많이 쓰는 방식을 순서대로 매긴 질문(사용자 자신) — 1순위뿐 아니라 전체 우선순위가 답변에 들어있다 / 슬라이더형 2개 — 하나는 이상형이 "내 사람에게만 잘하는 사람"부터 "누구에게나 두루 잘하는 사람"까지 중 어디에 가까웠으면 하는지(이상형에게 바라는 것), 다른 하나는 반대로 사용자 자신이 실제로 어느 쪽에 가까운지(사용자 자신)를 고른 질문 / 상황 제시형 4개 — 약속 취소·표정 변화·침묵 같은 구체적 상황에서 이상형이 어떻게 반응했으면 하는지를 고른 질문 3개(특성을 직접 물은 것보다 진한 신호다)와, 가까운 사람과 의견이 갈렸을 때 실제로 결과가 어떻게 됐는지(내 뜻대로/상대 뜻대로/절충/흐지부지 — 사용자 자신)를 고른 질문 1개 / 선호형 6개 — 성격, 가치관, 연애 방식, 라이프스타일, 끌리는 순간, 연락 스타일 등 여러 선택지 중 우선순위를 고르는 질문 / 양자택일형 5개 — 둘 중 하나를 고르는 우선순위 질문 / 경험·행동형 7개 — 가까운 관계에서 실제로 어땠는지 묻는 질문, 예: 사과 방식, 관계가 멀어질 때의 패턴, 연애 초반 스타일, 갈등 시 스트레스 대처, 돌아봤을 때의 아쉬움, 감정 기복, 예전과 달라진 점 / 짧은 자유 서술형 3개 — 그중 3개(관계가 멀어질 때·갈등 시 스트레스 대처·돌아봤을 때의 아쉬움) 바로 뒤에 붙어 "왜 그랬던 것 같은지"를 한 줄로 적는 질문, 건너뛸 수 있지만 답했다면 해당 경험 답변의 이유를 직접 설명하는 가장 근거가 확실한 재료다)과, 필수를 마친 뒤 선택적으로 더 답할 수 있는 심화 6문항(경험·행동형 2개 — 진지해지는 순간, 주변의 성취를 대하는 태도 등 더 세밀한 경험 질문 / 양자택일형 2개 / 선호형 2개). 즉 필수 문항만 답했더라도 경험·행동형 답변이 최소 7개는 항상 있다 — "질문: 돌아보면...", "질문: 가까웠던 관계가 멀어질 때..." 처럼 실제 경험을 묻는 질문에 대한 답은 사용자가 스스로를 돌아보고 준 재료이니, 몇 개가 있든 attractionPatterns와 특히 selfReflection에서 추측이 아니라 그 답을 직접 근거로 삼아 반영하라. 경험·행동형 질문 바로 뒤의 짧은 자유 서술(건너뛰지 않고 답한 경우)은 "왜 그랬는지"를 사용자가 직접 설명한 것이니, 있다면 그 경험 답변과 반드시 짝지어 해석하라 — 선택지만으로는 추측일 수밖에 없는 이유를 사용자가 스스로 밝힌 것이다. "요즘 마음 상태"와 "관계에서 주로 맡는 역할", 그리고 "내가 실제로 마음을 표현하는 방식"·"내가 가까운 사람에게만 잘하는지 두루 잘하는지"·"의견이 갈렸을 때 결국 어떻게 됐는지"에 대한 답도 selfReflection의 재료로 적극 활용하라 — 이상형 취향이 아니라 지금 사용자 자신을 직접 말해주는 답이라 다른 어떤 문항보다도 selfReflection에 곧바로 쓸 수 있다. 이 중 순위·슬라이더·상황 제시형에 있는 3개(마음 표현 방식/온도차/의견 대립 결과)는 각각 짝이 되는 이상형 선호 질문이 바로 근처에 있으니, 짝지어진 두 답이 같은 방향인지 어긋나는지를 selfReflection과 attractionPatterns에서 반드시 비교하라 — 어긋난다면("받고 싶은 방식과 실제로 주는 방식이 다르다", "이상형에게는 두루두루를 바라면서 본인은 내 사람 위주다" 같은) 그 간극 자체가 가장 좋은 재료다.
...
- 필수 37문항만 답해 경험·행동형 답변이 7개뿐인 경우에는 예측·변화·교차 문장을 쓰지 마라 — 근거 없는 추측이 된다. 지금 있는 답변만으로 확인 가능한 해석에 머물러라.
