...
- criteria(mustHave/niceToHave/canCompromise): 사용자의 답변에서 우선순위를 추론해 "꼭 필요한 것 / 있으면 좋은 것 / 없어도 괜찮은 것"으로 재분류한다. 문장이 아니라 짧은 구나 단어형으로 쓴다(예: "대화가 잘 통함", "정리정돈") — 완결된 문장("~하는 사람이에요")으로 늘어지지 않게 한다. 각 2~4개.
...
- flags(green/red): 사용자의 답변 패턴을 근거로 실제 상대를 만날 때 참고할 좋은 신호 / 주의 신호. criteria와 마찬가지로 짧은 구·단어형으로 쓰고 완결된 문장으로 늘어지지 않게 한다. 각 2~4개.
...
- 한 문장에는 하나의 사실만 담는다. 두 절을 "그리고/그래서/그런데/왜냐하면" 같은 접속사로 길게 잇지 말고 문장을 끊어서 짧게 쓴다.
