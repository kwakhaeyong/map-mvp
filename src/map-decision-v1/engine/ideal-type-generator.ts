...
퀴즈는 두 구간으로 나뉜다: 반드시 답하는 필수 문항(선호형·양자택일형 위주 — 외모, 성격, 가치관, 연애 방식, 라이프스타일, 소통 스타일, 끌리는 순간, 연애 속도 같은 취향과, 둘 중 하나를 고르는 우선순위 질문)과, 마쳤을 때 선택적으로 더 답할 수 있는 심화 문항(과거 연애에서 실제로 어땠는지를 묻는 경험·행동형 질문 — 예: 연애가 끝날 때 패턴, 스트레스를 푸는 방식, 아쉬웠던 점). "질문: 돌아보면...", "질문: 연애가 끝날 때..." 처럼 실제 경험을 묻는 질문에 대한 답이 있다면, 그건 사용자가 스스로를 돌아보고 준 재료이니 attractionPatterns와 특히 selfReflection에서 최우선으로, 구체적으로 반영하라. 그런 답이 없다면(필수 문항만 답한 경우) 억지로 지어내지 말고 선택지 답변만으로 자연스럽게 구성하라.
...
- attractionPatterns: ★핵심★. 사용자가 고른 선택지들을 가로질러 반복되는 끌림의 패턴을 짚어준다. 단순 요약이 아니라 "왜 그런 것에 끌리는지"에 대한 해석을 담는다. 보통 2~3개, 경험·행동형 답변까지 있으면 거기서 나온 패턴을 하나 더 추가해 최대 4개까지 써도 된다.
...
- selfReflection(whatYouOffer/whatToImprove): ★가장 중요한 항목★. 이상형에 대한 답변을 거꾸로 뒤집어서 사용자 자신에 대한 통찰을 준다 — "이런 사람에게 끌린다는 것은, 본인은 이런 면을 가지고 있거나 이런 걸 줄 수 있는 사람일 가능성이 높다"는 식의 역추론. whatToImprove는 사용자가 이상형에게 바라는 것과 본인의 현재 모습 사이의 간극에서 나오는 보완점을 짚는다. 각 보통 2~3개, 경험·행동형 답변(연애 패턴·아쉬웠던 점 등)이 있으면 그 실제 경험에 근거한 항목을 하나 더 추가해 최대 4개까지 써도 된다 — 이 경우엔 추측이 아니라 사용자가 직접 답한 패턴을 근거로 삼아라.
