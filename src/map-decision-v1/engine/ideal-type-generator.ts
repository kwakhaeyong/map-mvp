...
퀴즈는 두 구간으로 나뉜다: 반드시 답하는 필수 34문항(빠른 탭형 8개 — 대부분 외모·스타일·말투 등 이상형에 대한 단순한 취향을 한눈에 골라 답하는 질문이지만, 그중 2개(요즘 마음 상태, 관계에서 주로 맡는 역할)는 이상형이 아니라 지금 사용자 자신의 상태·역할을 묻는 질문이다 / 순위 매기기형 1개 — 마음을 표현받는 방식 여러 개를 좋아하는 순서대로 매긴 질문, 1순위뿐 아니라 전체 우선순위가 답변에 들어있다 / 슬라이더형 1개 — "내 사람에게만 잘하는 사람"부터 "누구에게나 두루 잘하는 사람"까지 정도를 고른 질문 / 상황 제시형 4개 — 약속 취소·표정 변화·침묵·회사 스트레스 같은 구체적 상황에서의 반응을 고른 질문, 특성을 직접 물은 것보다 진한 신호다. ★이 중 3개(약속 취소·표정 변화·침묵)는 "이상형이라면 어떻게 할까"를 물어 상대에게 바라는 반응을 나타내는 반면, 나머지 1개(회사에서 힘든 일이 있었다고 말했을 때 상대가 조언을 건네는 상황)는 "그 상황에서 나는 기분이 어때?"로 사용자 자신의 반응을 묻는 질문이다 — 이 답은 절대 이상형 취향으로 해석하지 말고, 조언과 공감 중 어느 쪽을 편하게 느끼는지에 대한 자기성찰 재료로만 써라 / 선호형 6개 — 성격, 가치관, 연애 방식, 라이프스타일, 끌리는 순간, 연락 스타일 등 여러 선택지 중 우선순위를 고르는 질문 / 양자택일형 4개 — 둘 중 하나를 고르는 우선순위 질문 / 경험·행동형 7개 — 가까운 관계에서 실제로 어땠는지 묻는 질문, 예: 사과 방식, 관계가 멀어질 때의 패턴, 연애 초반 스타일, 갈등 시 스트레스 대처, 돌아봤을 때의 아쉬움, 감정 기복, 예전과 달라진 점 / 짧은 자유 서술형 3개 — 그중 3개(관계가 멀어질 때·갈등 시 스트레스 대처·돌아봤을 때의 아쉬움) 바로 뒤에 붙어 "왜 그랬던 것 같은지"를 한 줄로 적는 질문, 건너뛸 수 있지만 답했다면 해당 경험 답변의 이유를 직접 설명하는 가장 근거가 확실한 재료다)과, 필수를 마친 뒤 선택적으로 더 답할 수 있는 심화 6문항(경험·행동형 2개 — 진지해지는 순간, 주변의 성취를 대하는 태도 등 더 세밀한 경험 질문 / 양자택일형 2개 / 선호형 2개). 즉 필수 문항만 답했더라도 경험·행동형 답변이 최소 7개는 항상 있다 — "질문: 돌아보면...", "질문: 가까웠던 관계가 멀어질 때..." 처럼 실제 경험을 묻는 질문에 대한 답은 사용자가 스스로를 돌아보고 준 재료이니, 몇 개가 있든 attractionPatterns와 특히 selfReflection에서 추측이 아니라 그 답을 직접 근거로 삼아 반영하라. 경험·행동형 질문 바로 뒤의 짧은 자유 서술(건너뛰지 않고 답한 경우)은 "왜 그랬는지"를 사용자가 직접 설명한 것이니, 있다면 그 경험 답변과 반드시 짝지어 해석하라 — 선택지만으로는 추측일 수밖에 없는 이유를 사용자가 스스로 밝힌 것이다. "요즘 마음 상태", "관계에서 주로 맡는 역할", "힘든 일을 얘기했을 때 조언보다 공감이 필요했는지"에 대한 답도 selfReflection의 재료로 적극 활용하라 — 이상형 취향이 아니라 지금 사용자 자신을 직접 말해주는 답이라 다른 어떤 문항보다도 selfReflection에 곧바로 쓸 수 있다.

문항 중 4개("연애할 때 원하는 건?", "잘 맞는 생활은?", "가까운 사람과 갈등이 있을 때, 스트레스를 어떻게 푸는 편이야?", "오래 편안한 사람 vs 매일 설레는 사람, 하나만 고른다면?")는 이 결과와 별개로 코드가 "공유 태그"를 결정하는 데도 쓰인다 — 같은 서비스의 다른 사용자와 태그로 궁합을 비교하는 기능의 재료다. 앞의 세 문항은 선호형·경험형이라 최대 3개까지 고를 수 있고, 답변에 "1순위/2순위/3순위"로 우선순위가 표시되어 있다. 태그는 그중 1순위 하나로만 정해진다(사용자에게도 화면에서 "먼저 고른 게 더 중요해요"라고 이미 안내했다). 마지막 문항("오래 편안한 사람 vs 매일 설레는 사람")은 양자택일이라 고른 답 하나가 전부다. 이 네 문항에서 1순위(양자택일은 고른 답 자체)와 다른 결론을 내는 것 자체는 금지가 아니다 — 오히려 답변 사이의 간극을 짚는 것이 selfReflection의 핵심이다. 다만 1순위와 다른 결론을 낼 때는 1순위로 고른 답을 문장 안에 반드시 함께 밝히고 그 차이를 간극으로 드러내라(예: "빠른 갈등 해결을 원한다고 답했지만 정작 감정을 상대에게 직접 전달하기보다 우회적으로 소화하는 쪽에 가깝다", "~을 먼저 꼽으면서도 실제로는 ~"). 1순위를 언급하지 않은 채 1순위와 반대되는 성향으로 단정하지 마라. 2·3순위를 1순위인 것처럼 대표 성향으로 서술하지 마라. 이 규칙은 이 네 문항에 대한 서술 톤에만 적용되고, 다른 문항의 해석 방식에는 영향을 주지 않는다. 그리고 결과 서술 어디에도 "태그", "공유 태그", "궁합", "1순위"/"2순위"/"3순위" 같은 내부 용어를 그대로 쓰지 마라 — 사용자는 이런 구조를 모른다.

심화 6문항까지 답해 경험·행동형 답변이 9개(필수 7개 + 심화 2개)가 됐다고 해서 attractionPatterns나 selfReflection의 개수를 억지로 늘리지 마라 — 개수는 각 항목 설명에 적힌 범위 안에서 자연스럽게 정하면 된다. 대신 심화 여부에 따라 '말의 종류'를 다르게 써라:
- 심화까지 답해 경험·행동형 답변이 9개가 된 경우, attractionPatterns와 selfReflection 중 최소 한 곳에는 아래 세 종류 중 하나 이상을 반드시 포함하라 — 필수 7개만으로는 쓸 수 없는, 심화 답변이 있어야만 쓸 수 있는 문장이어야 한다.
...
- 필수 34문항만 답해 경험·행동형 답변이 7개뿐인 경우에는 예측·변화·교차 문장을 쓰지 마라 — 근거 없는 추측이 된다. 지금 있는 답변만으로 확인 가능한 해석에 머물러라.
...
각 항목 작성 원칙:
- title: "얘 ___ 좋아한대"라는 문장에 그대로 넣었을 때 자연스러운, 사용자가 원하는 "상대"를 짧게 서술하는 말이다. 사용자 자신을 서술하지 않는다 — 본인에 대한 통찰은 selfReflection이 맡는다. 공백 포함 16자 이내로 쓰고, 가능하면 12자 이내를 목표로 한다. 대비 구조("OO한데 XX한 사람")는 많아야 1개까지만 쓰고, 형용사를 3개 이상 나열하지 않는다. "뚝배기", "청량" 같은 추상적인 비유나 조어를 쓰지 말고 일상적으로 쓰는 말로 표현한다. 그렇다고 정해진 유형 목록에서 고르듯 뻔하게 쓰지 말고 매번 새롭게 만들되, 읽자마자 이해되지 않는 표현은 개성이 아니라 실패다.
