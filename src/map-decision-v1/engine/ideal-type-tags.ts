...
export const TAG_CATEGORIES: TagCategory[] = [
  ...
  },
  {
    ...
    mapping: {
      ...
      // 일할 때의 나 전용(신규 4개, 2026-08-10) — 원래는 나 소개 전용
      // 4개와 라벨을 그대로 공유했으나, 문항을 업무 맥락으로 바꾸면서
      // (topics.ts의 work.experienceStressResponse 참고) 라벨 문자열도
      // 달라져 더 이상 위 4개로 안 잡힌다. 기존 4개와 문자열이 겹치지
      // 않게 새로 지었다(TAG_TO_CATEGORY가 태그 문자열을 키로 쓰는
      // Map이라, 겹치면 나중에 등록되는 쪽만 남고 먼저 등록된 쪽이
      // 조회 불가능해진다).
      "그 자리에서 얘기하는 편": "#정면돌파형",
      "혼자 삭이고 넘기는 편": "#혼자소화형",
      "필요한 말만 하며 거리 두는 편": "#선긋기형",
      "동료에게 털어놓는 편": "#동료공유형",
    },
  },
  {
...
// 참조하지만, 사전은 주제별로 따로 둔다 — selfIntro·friendship은 어투
// (관계 맥락)만 다를 뿐 문항 라벨은 그대로 공유하고, work는 2026-08-10
// 문항 자체를 업무 맥락으로 바꾸면서(topics.ts 참고) 라벨 문자열도
// 달라져 더 이상 공유하지 않는다.
...
const WORK_STATUS_LABELS: Record<string, string> = {
  "그 자리에서 얘기하는 편": "지금은 문제를 바로 꺼내 놓는 자리에 있어요",
  "혼자 삭이고 넘기는 편": "지금은 안에서 소화하고 넘어가는 자리에 있어요",
  "필요한 말만 하며 거리 두는 편": "지금은 거리를 두고 판단을 미루는 자리에 있어요",
  "동료에게 털어놓는 편": "지금은 주변에 나누며 정리하는 자리에 있어요",
};
