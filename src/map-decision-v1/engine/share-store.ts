...
export type SharedResultRecord = {
  ...
  createdAt: string;
  result: unknown;
  // 예전에 심화(선택) 문항까지 답하고 만든 결과인지 — 공유 페이지
  // (/r/[id])에서 "🔍 심층 분석 포함" 배지를 보여줄지 판단하는 데 쓴다.
  // 이상형(quizVersion 11)·나 소개(quizVersion 2) 둘 다 심화 경로를
  // 없애 이제 항상 undefined다(예전에 심화까지 답해 저장된 기존 링크는
  // "deep"으로 남아있어 배지가 계속 보인다). 이 필드가 생기기 전에
  // 저장된 링크는 undefined로 읽히고, 그냥 배지 없이 보여주면 되므로
  // 별도 이관이 필요 없다.
