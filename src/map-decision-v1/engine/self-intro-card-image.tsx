// 나 소개·성격의 1080×1350 초대장 컨셉 card.png satori 렌더링 로직.
// 이상형(ideal-type-card-image.tsx)은 4가지 테마(purple/navy/colorBlock/
// invitation)를 전부 지원하는 큰 파일이라 그대로 재사용하기보다, 이
// 카드는 스테이지 2 지시대로 "초대장 컨셉 하나만" 필요해서 그 파일을
// 건드리지 않고 여기 새로 작성했다. 폰트 로딩·PNG 압축·카드 크기
// 상수는 이미 export돼 있는 이상형 쪽 유틸을 그대로 가져다 쓴다 —
// 종이 질감 로더·안전 자르기 같은 나머지 helper만 최소로 복제했다
// (그 함수들은 이상형 파일 안에 private로만 있어 재사용할 수 없다).
...
import type { SelfIntroResult } from "../types";
...
// footer와는 떨어져 보인다. 그것만으로는(Zone이 justifyContent:center라
// 오직 존 높이 차이에만 의존) 간격 차이가 oneLiner 줄 수(1~3줄)에 따라
// 들쭉날쭉하고 눈에 띄게 크지 않아서, Zone에 justifyContent 오버라이드
// (style prop, ideal-type-card-image.tsx의 InvitationZone과 같은 방식)를
// 추가해 OneLiner는 자기 존 아래쪽에, StatusLabel은 자기 존 위쪽에
// 붙인다 — 둘 사이 간격이 항상 거의 0이 되어 oneLiner 줄 수와 무관하게
// "바로 아래"가 유지된다. Footer는 반대로 자기 존 아래쪽(bottomMargin
...
// 담는다(이상형의 초대장 카드와 동일한 범위 — 자기성찰 텍스트는 카드에
// 넣지 않는다).
export function buildSelfIntroCardElement(result: SelfIntroResult) {
