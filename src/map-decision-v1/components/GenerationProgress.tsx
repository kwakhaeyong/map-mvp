...
//
// 아래 세 상수는 프로덕션 로그로 실측한 생성 시간을 기준으로 잡았다.
// 프롬프트에서 효과가 확인되지 않던 글자 수 상한 지시를 제거한 뒤(#216)
// 생성 시간이 165초에서 108초로 줄어서, 이 값들도 108초 기준으로 다시
// 맞췄다(2026-08). 생성 시간이 다시 크게 바뀌면(프롬프트·모델·재시도
// 구조 변경 등) 이 값들도 함께 재조정해야 한다 — 실측치와 어긋난 채로
// 방치하면 문구가 다 지나간 뒤 남는 시간이 다시 길어지거나 짧아진다.
//
// STAGE_INTERVAL_MS: 5단계 × 22초 = 110초로, 실측 108초와 거의 같다.
// 그래서 대부분의 생성은 5단계를 다 도는 동안 끝나거나, 마지막 단계에
// 막 들어선 지 얼마 안 돼 끝난다 — 문구가 다 지나갔는데 마지막 문구만
// 한참 멈춰 있는 상태를 피한다.
const STAGE_INTERVAL_MS = 22_000;
...
export function GenerationWaitCard({
  ...
  // 5단계 문구가 다 끝나 마지막 문구에 멈춰 있는 정지 구간(88초 이후 —
  // STAGE_INTERVAL_MS 22초 × 5단계). 실측 생성 시간이 108초라 대부분의
  // 생성이 이 구간에 짧게(20초 안팎) 머물다 끝난다. 이 구간에서만 태그를
  // 하나씩 순서대로 옅게 강조해 "화면이 멈추지 않았다"를 보여준다 —
  // 진행률이나 남은 시간을 아는 척하지 않는 순수 시각 신호다.
