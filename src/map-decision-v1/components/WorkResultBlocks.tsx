...
// 모음. FriendshipResultBlocks.tsx와 같은 원리이지만, 이상형·나 소개·
// 친구 코드를 건드리지 않기 위해 별도 파일로 새로 작성했다 — 라이브
// 결과 화면(WorkCard.tsx)과 공유 읽기 전용 화면(app/r/[id]/page.tsx)
// 둘 다에서 재사용한다.
//
...
// 맞췄다: "모습"은 MatrixSection 제목("나의 여러 모습")에서, "다음
// 행동"은 RoadmapSection의 현재 제목과 그대로 맞췄다.
const NAV_ITEMS: Array<{ key: string; label: string }> = [
  { key: "drivers", label: "동력" },
...
// WorkDriversSection 주석과 같다(뷰포트 640px 기준 접두사가 384px
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-3">
        <p className="text-xs font-extrabold uppercase tracking-[0.14em] text-text-secondary">내가 발휘하고 있는 것</p>
...
export function WorkResultBlocks({
  ...
  return (
    <>
      ...
      <div ref={(el) => { sectionRefs.current.drivers = el; }} className="scroll-mt-16">
