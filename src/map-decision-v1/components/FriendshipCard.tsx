...
import { FriendshipResultBlocks } from "./FriendshipResultBlocks";
import { ShareStatusCard, useShareResult } from "./ShareResult";
import { ImageSaveModal, useImageShare } from "./ImageShare";
import { Button, Card } from "./ui/primitives";

// 친구·인간관계 결과의 실제 구성 순서(관계 발견 엔진 프롬프트 기준:
// 패턴 → 매트릭스 배치 → 자기성찰 → 나머지 필드 정리)와 대응하는 대기
// 문구. 이상형(IdealTypeCard.tsx)·나 소개(SelfIntroCard.tsx)와 같은
// 원리 — 실제로 나오는 항목만 순서대로 쓴다.
const FRIENDSHIP_GENERATION_STAGES = [
  "답변을 읽고 있어요",
  "반복되는 패턴을 찾는 중이에요",
  "나의 여러 모습을 배치하고 있어요",
  "당신에 대한 통찰을 정리하고 있어요",
...
function FriendshipCardBody({
  ...
  const result = session.friendshipResult!;

  // 친구·인간관계는 심화(선택) 경로가 없는 주제라 quizDepth를 넘기지
  ...
  const { shareState, shareError, sharedUrl, canNativeShare, share, copyLink, ensureShareUrl } = useShareResult({
    ...
    result,
    signature: session.friendshipResultSignature,
  ...
  return (
    <div className="flex flex-col gap-3">
      <FriendshipResultBlocks result={result} />
...
// 이상형·나 소개(IdealTypeCard.tsx/SelfIntroCard.tsx)와 같은 이유로 이
// 화면의 배경만 크림 톤(bg-background)으로 맞춘다 — body 자체를 바꾸지
// 않아 퀴즈 진행 화면·랜딩·진로 결과 화면은 전혀 영향받지 않는다.
export function FriendshipCard({
  ...
  const generate = () => {
    ...
    fetch("/api/generate-friendship-result", {
      method: "POST",
      ...
      .then((data) => {
        ...
        setSession((previous) => ({ ...previous, friendshipResult: data.result, friendshipResultSignature: data.signature, updatedAt: now() }));
  ...
  useEffect(() => {
    if (session.friendshipResult || attemptedRef.current) return;
  ...
  return (
    <main className="min-h-dvh bg-background px-4 py-4 pb-safe-bottom pt-safe-top text-text-primary">
      <div className="mx-auto flex w-full max-w-sm flex-col gap-3">
        ...
        {session.friendshipResult ? (
          <FriendshipCardBody session={session} setSession={setSession} onReset={onReset} />
        ) : generationState === "loading" ? (
          <GenerationWaitCard
            key={attempt}
            stages={FRIENDSHIP_GENERATION_STAGES}
            onRetry={generate}
