      ...
      <section className="map-container">
        <header className="rounded-large border border-border bg-surface p-6 shadow-floating sm:p-8 print:shadow-none">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2 print:hidden"><Button onClick={onContinue}>더 이야기하기</Button><Button variant="secondary" onClick={() => window.print()}>저장 / 내보내기</Button><Button variant="ghost" onClick={safeReset}>{session.isDemo ? "처음으로" : "새 MAP 만들기"}</Button></div>
          </div>
        ...
        <div className="mt-6"><MapCanvas session={session} result /></div>
        ...
        <ResultActionBar className="print:hidden"><Button onClick={onContinue}>더 이야기하기</Button><Button variant="secondary" onClick={onContinue}>특정 내용 수정하기</Button><Button variant="secondary" onClick={() => window.print()}>저장 / 내보내기</Button>{session.isDemo ? <Button variant="secondary" onClick={onRealStart}>직접 해보기</Button> : null}</ResultActionBar>
      </section>
    </main>
  );
}
...
function ResultCard({ title, body }: { title: string; body: string }) {
  return <ReflectionCard><h2 className="text-lg font-extrabold">{title}</h2><p className="mt-3 whitespace-pre-line break-keep font-medium leading-7 text-text-secondary">{body}</p></ReflectionCard>;
}
