...
// 일할 때의 나 결과의 실제 구성 순서(일 발견 엔진 프롬프트 기준: 패턴 →
// 매트릭스 배치 → 자기성찰 → 나머지 필드 정리)와 대응하는 대기 문구.
// 이상형(IdealTypeCard.tsx)·나 소개(SelfIntroCard.tsx)·친구·인간관계
// (FriendshipCard.tsx)와 같은 원리 — 실제로 나오는 항목만 순서대로 쓴다.
const WORK_GENERATION_STAGES = [
  "답변을 읽고 있어요",
  "반복되는 일 패턴을 찾는 중이에요",
  "업무 속 여러 모습을 배치하고 있어요",
  "당신에 대한 통찰을 정리하고 있어요",
  "마무리하고 있어요",
];
...
function WorkCardBody({
  ...
  // 일할 때의 나는 심화(선택) 경로가 없는 주제라 quizDepth를 넘기지
  ...
  const { shareState, shareError, sharedUrl, canNativeShare, share, copyLink, ensureShareUrl } = useShareResult({
    ...
    result,
    signature: session.workResultSignature,
  ...
  return (
    <div className="flex flex-col gap-3">
      <WorkResultBlocks result={result} />
...
// 이상형·나 소개·친구(IdealTypeCard.tsx/SelfIntroCard.tsx/FriendshipCard.tsx)와
...
export function WorkCard({
  ...
  useEffect(() => {
    if (session.workResult || attemptedRef.current) return;
